...
extension ImageBrowser{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell{
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cellId", for: indexPath) as! BrowserCell
        ...
        if ((self.delegate?.getTheThumbnailImage(indexPath.row)) != nil) {
            ...
            thumnailSize = CGSize(width: ScreenWidth * UIScreen.main.scale, height: ScreenWidth * UIScreen.main.scale)
            imageManager.requestImage(for: asset, targetSize: thumnailSize, contentMode: .aspectFill, options: nil) { img, _ in
                    cell.setImageWithImage(img!, placeholderImage: self.defaultImage, defaultImage: self.defaultImage)
            }
